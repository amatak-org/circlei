import { Config, OsInfo, Transient } from '@panel/common';
import {
  faApple,
  faCentos,
  faFedora,
...
export const ServerWidget: FC<ServerWidgetProps> = ({ data, config }) => {
  ...
  const ghBtn = (
    <Link
      ...
      href='https://github.com/amatak-org/server'
      target='_blank'
  ...
  return (
    <Container>
      <ButtonsContainer>
        {config.show_panel_version === 'icon_hover' ? (
          <Tooltip placement='right' title={data.panel_version}>
            {ghBtn}
          </Tooltip>
        ) : (
          ghBtn
      ...
      <Heading>
        {config?.show_host && host ? (
          <>
            <Appendix>Amatak Panel.</Appendix>
            <ServerName>{host}</ServerName>
          </>
        ) : (
          <StandaloneAppendix>Amatak Panel.</StandaloneAppendix>
        )}
      </Heading>
      ...
      <StyledInfoTable
        ...
        infos={toInfoTable(config.os_label_list, {
          ...
          panel_version: { label: 'Amatak Panel Version', value: data.panel_version },
        })}
        page={0}
        itemsPerPage={7}
      />
